...
/**
 * Assemble the full report payload from a report configuration.
 * Fetches all required data from Supabase and transforms it for the renderer.
 *
 * @throws If the survey has no calculated scores
 */
export async function assembleReportPayload(config: ReportConfig): Promise<ReportPayload> {
  ...
  // Map segment scores, filtering by anonymity threshold
...
/** Fetch organization details via the survey's organization_id */
async function fetchOrganizationForSurvey(surveyId: string): Promise<OrgInfo> {
  ...
  return {
    ...
    cccLogoUrl: null, // Loaded from app config, not per-org
    colors: brandColors,
  };
}

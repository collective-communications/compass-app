...
import { severity, greyscale, textColors } from '@compass/tokens';
      ...
      {Object.entries(grouped).map(([severity, items]) => (
        <div key={severity} style={{ marginBottom: '20px' }}>
          ...
          >
            ...
            />
            <span style={{ fontSize: '11pt', fontWeight: 600, color: greyscale[900] }}>
              {SEVERITY_LABELS[severity] ?? severity}
            </span>
            <span style={{ fontSize: '9pt', color: greyscale[400] }}>
              ({items.length})
            </span>
          </div>
          ...
          {items.map((rec, idx) => (
            ...
            >
              ...
              <p style={{ fontSize: '9pt', color: textColors.tertiary.light, marginBottom: '8px', lineHeight: 1.5 }}>
                {rec.description}
              </p>

              {rec.actions.length > 0 && (
                <div style={{ marginTop: '4px' }}>
                  <p style={{ fontSize: '8pt', fontWeight: 600, color: greyscale[400], textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '4px' }}>
                    Actions
                  </p>

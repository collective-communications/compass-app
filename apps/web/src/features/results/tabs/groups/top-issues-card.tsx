...
export function TopIssuesCard({ questions, limit = 3 }: TopIssuesCardProps): ReactElement {
  ...
  if (sorted.length === 0) {
    return (
      <div className="rounded-lg border border-[#E5E4E0] bg-white p-6">
        <h3 className="text-sm font-semibold text-[#424242]">Top Issues</h3>
        <p className="mt-2 text-xs text-[#757575]">No question data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-[#E5E4E0] bg-white p-6">
      <h3 className="mb-3 text-sm font-semibold text-[#424242]">
        Top Issues
      </h3>
      <ol className="flex flex-col gap-3">
        {sorted.map((q, idx) => (
          <li key={q.questionId} className="flex items-start gap-2">
            <span className="flex size-5 shrink-0 items-center justify-center rounded-full bg-[#F5F5F5] text-xs font-medium text-[#757575]">
              {idx + 1}
            </span>
            <div className="flex flex-col gap-0.5">
              <p className="text-sm text-[#424242]">{q.questionText}</p>

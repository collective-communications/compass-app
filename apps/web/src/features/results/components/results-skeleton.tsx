...
function SkeletonCard({ className = '' }: { className?: string }): ReactElement {
  return (
    <div
      className={`animate-pulse rounded-lg border border-[#E5E4E0] bg-white p-6 ${className}`}
    >
      <div className="mb-4 h-4 w-1/3 rounded bg-[#E5E4E0]" />
      <div className="mb-2 h-3 w-full rounded bg-[#E5E4E0]" />
      <div className="mb-2 h-3 w-5/6 rounded bg-[#E5E4E0]" />
      <div className="h-3 w-2/3 rounded bg-[#E5E4E0]" />
    </div>
  );
}
      ...
      <div className="flex flex-col gap-4 lg:w-[65%]">
        ...
        <div className="flex gap-2">
          {Array.from({ length: 5 }, (_, i) => (
            <div
              key={i}
              className="h-8 w-24 animate-pulse rounded-full bg-[#E5E4E0]"
            />
          ))}
        </div>

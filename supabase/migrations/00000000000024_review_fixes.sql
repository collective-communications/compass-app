...
ALTER TABLE email_log ADD COLUMN survey_id UUID REFERENCES surveys(id) ON DELETE SET NULL;
...
-- Recreate IVFFlat index with lists = 10 for early dataset size
-- NOTE: Increase lists to sqrt(row_count) when table exceeds 10k rows

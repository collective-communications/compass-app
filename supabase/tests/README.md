...
| File | Purpose |
| --- | --- |
| `helpers/fixtures.sql` | Not a test — `\ir`-included at the top of every test file. Installs the `pgtap` extension (idempotent) and registers shared helper functions (`create_test_org`, `create_test_user`, `create_test_survey`, `create_test_deployment`, `create_test_question`, plus `set_auth_uid` / `set_session_token_header` / `clear_auth` for role-switch convenience). Lives under `helpers/` so Supabase CLI's top-level glob (`supabase/tests/*.sql`) does not discover it as a test. |

...
function makeRequest(token?: string): Request {
  return new Request('https://edge.test/score-survey', {
...
describe('score-survey authorize', () => {
  ...
  test('valid required org_members membership is allowed', async () => {
    const { client } = makeClient({ data: [{ role: 'ccc_admin' }], error: null });
  ...
  test('admin membership is honored when user has multiple memberships', async () => {
